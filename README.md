...
실제 양조 라인의 작업 흐름을 반영하여 **생산계획 -> 작업 지시 -> 생산공정 실행 및 실시간 모니터링 -> 생산실적관리** 까지 <br/>
연결되어 프로그램을 구현하였으며  **현장 중심의 실질적인 생산 관리 UX 구현**에 초점을 맞추었습니다.<br/>


### 🔧 담당 기능: 생산 공정 관리 흐름 (10개 공정 구현)

생산 계획에 따라 발행된 **작업지시서(LOT)** 를 기반으로
분쇄 → 당화 → 여과 → 끓임 → 냉각 → 발효 → 숙성 → 숙성후여과 -> 탄산 → 패키징에 이르는
전체 생산 공정을 연동하며 필요한 자재정보 및 공정간 정보 자동으로 로드되며 
온도별 설비 작동을 최대한 구현하기 위해 노력 하였습니다.
