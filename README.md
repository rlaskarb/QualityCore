...
생산계획 -> 작업 지시 -> 공정 실행 (실시간 모니터링) -> 생산실적관리 의 진행이 이어지는 프로그램을 구현했습니다.<br/>


🔧 담당 기능: 생산 공정 관리 전반 (9개 공정 구현)

생산 계획에 따라 발행된 **작업지시서(LOT)**를 기반으로
분쇄 → 당화 → 여과 → 끓임 → 냉각 → 발효 → 숙성 → 숙성후여과 -> 탄산 → 패키징에 이르는
전체 생산 공정을 연동하고, 공정 간 자동 정보 이관, 상태 제어, 타이머 기반 처리를 구현하였습니다. 

<br/>

✅ 1. 공정별 정보 자동 불러오기 & UI 시각화
작업지시서 선택 시, 해당 공정에 필요한 자재 정보, 투입량, 설정값(온도, 압력 등) 자동 로드

각 공정 진행 시, 모달/타이머 UI로 공정 상태 시각화

분쇄 간격, 물 대비 비율, 홉 투입량 등 조건 자동 계산 및 검증 로직

✅ 2. 공정 간 연동 로직
공정 완료 후 다음 공정으로 자동 연계되며, 이전 단계 결과값이 다음 공정에 자동 반영
(예: 여과 후 워트량 → 끓임공정 기준량, 당도 → 발효 기준값 등)

✅ 3. 실시간 수율 및 손실량 계산
각 공정별 손실률, 회수량, 비율 계산을 자동화
(예: 여과 손실 5%, 끓임 손실량, 곡물 흡수량 계산 등)

작업자는 시스템이 제안하는 기준값을 확인하고 수동 보정 가능
...
단순한 정보 나열이 아닌, **현장 중심의 실질적인 생산 관리 UX 구현**에 초점을 맞추었습니다. <br/> 
생산계획 -> 작업 지시 -> 공정 실행 (실시간 모니터링) -> 생산효율.<br/>

특히 저는 분쇄부터 당화, 여과, 끓임, 냉각, 발효, 숙성, 탄산, 패키징까지 전체 생산공정 관리 기능을 구현하며, <br/>
각 공정별로 작업지시 기반 자동 정보 로딩, 실시간 데이터 연산, 공정별 타이머 및 모달 제어,
품질 측정 기반 판단 로직 등을 설계 및 개발했습니다.
